/**
 * @file ble_client.c
 * @brief BLE central (GATT client) — connects to XPRS peripheral, exchanges APRS
 *
...
// XPRS BLE UUIDs (must match xprs_ble.c)
#define XPRS_SVC_UUID16       0xFFE0
#define XPRS_CHAR_WRITE16     0xFFF1
#define XPRS_CHAR_NOTIFY16    0xFFF2
...
// ============================================================================
// RX reassembly — mirrors ble_process_peer_buffer() from xprs_ble.c
// ============================================================================
 ...
 * Exact same algorithm as ble_process_peer_buffer() in xprs_ble.c.
 */
                       ...
                       const struct ble_gatt_chr *chr, void *arg)
{
    if (error->status == 0 && chr) {
        ...
        if (uuid16 == XPRS_CHAR_WRITE16) {
            s_write_handle = chr->val_handle;
        } else if (uuid16 == XPRS_CHAR_NOTIFY16) {
...
static int ble_client_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC: {
        ...
        for (int i = 0; i < fields.num_uuids16; i++) {
            if (ble_uuid_u16(&fields.uuids16[i].u) == XPRS_SVC_UUID16) {
                found = true;
                break;
            }
        }
        ...
        if (!found && fields.mfg_data_len >= 3 &&
            ...
            fields.mfg_data[2] == XPRS_BLE_MARKER) {
            found = true;
        }
        ...
        // firmware advertises "XPRS". Dropping the old one would make the
