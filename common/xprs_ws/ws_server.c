...
{
    if (!xprs_mesh_is_connected()) return;
    ...
    uint32_t ip = 0;
    xprs_mesh_get_external_ap_ip_addr(&ip);
    ...
    size_t node_count = 0;
    xprs_mesh_get_nodes(nodes, 20, &node_count);
    ...
    for (size_t i = 0; i < node_count; i++) {
        if (memcmp(nodes[i].mac, local_mac, 6) != 0) {
            xprs_mesh_send_to_node(nodes[i].mac, &msg, sizeof(msg));
        }
    }
