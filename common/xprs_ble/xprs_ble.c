...
static const char *TAG = "xprs_ble";

#define XPRS_BLE_SERVICE_UUID16          0xFFE0
#define XPRS_BLE_CHAR_WRITE_UUID16       0xFFF1
#define XPRS_BLE_CHAR_NOTIFY_UUID16      0xFFF2
#define XPRS_BLE_CHAR_STATUS_UUID16      0xFFF3

#define XPRS_BLE_MARKER                  0x3E
#define XPRS_BLE_COMPANY_ID              0xFFFF
...
#if BOARD_MODEL == MODEL_KV4P
#define XPRS_BLE_MAX_CONNECTIONS         1
#define XPRS_BLE_RX_BUFFER_SIZE          1024
#else
#define XPRS_BLE_MAX_CONNECTIONS         2
#define XPRS_BLE_RX_BUFFER_SIZE          4096
#endif
#define XPRS_BLE_MAX_PATH_LEN            192
#define XPRS_BLE_MAX_QUERY_LEN           192
#define XPRS_BLE_MAX_ROOM_LEN            48
#define XPRS_BLE_MAX_AUTHOR_LEN          24
#define XPRS_BLE_MAX_MESSAGE_LEN         MESH_CHAT_MAX_MESSAGE_LEN
#define XPRS_BLE_MAX_NOTIFY_CHUNK        180
// Avoid large heap allocations on low-memory boards (KV4P).
#define XPRS_BLE_CHAT_FETCH_MAX          2
#define XPRS_BLE_CHAT_JSON_MAX_LEN       1024
...
static xprs_ble_peer_t s_peers[XPRS_BLE_MAX_CONNECTIONS];
...
#if CONFIG_BT_ENABLED
static const ble_uuid16_t s_service_uuid = BLE_UUID16_INIT(XPRS_BLE_SERVICE_UUID16);
static const ble_uuid16_t s_write_uuid = BLE_UUID16_INIT(XPRS_BLE_CHAR_WRITE_UUID16);
static const ble_uuid16_t s_notify_uuid = BLE_UUID16_INIT(XPRS_BLE_CHAR_NOTIFY_UUID16);
static const ble_uuid16_t s_status_uuid = BLE_UUID16_INIT(XPRS_BLE_CHAR_STATUS_UUID16);
#endif
...
static xprs_ble_peer_t *ble_find_peer(uint16_t conn_handle)
{
    for (size_t i = 0; i < XPRS_BLE_MAX_CONNECTIONS; i++) {
...
static xprs_ble_peer_t *ble_alloc_peer(uint16_t conn_handle)
{
    xprs_ble_peer_t *peer = ble_find_peer(conn_handle);
    if (peer) {
        return peer;
    }
    ...
    for (size_t i = 0; i < XPRS_BLE_MAX_CONNECTIONS; i++) {
        if (!s_peers[i].active) {
            ...
            s_peers[i].rx_buffer = (uint8_t *)malloc(XPRS_BLE_RX_BUFFER_SIZE);
...
static char *ble_build_status_body(void)
{
    ...
    int written = snprintf(
        buffer,
        sizeof(buffer),
        "{\"service\":\"xprs-esp32\",\"status\":\"online\",\"callsign\":\"%s\",\"board\":\"%s\"}",
        callsign,
        BOARD_NAME
    );
...
static bool ble_query_get_long(const char *query, const char *key, long *value_out)
{
    ...
    char copy[XPRS_BLE_MAX_QUERY_LEN] = {0};
                                 ...
                                 char *query_out, size_t query_len)
{
    ...
    char normalized[XPRS_BLE_MAX_PATH_LEN] = {0};
    if (input[0] == '/') {
...
static bool ble_parse_messages_path(const char *path, char *room_out, size_t room_len)
{
    ...
    char copy[XPRS_BLE_MAX_PATH_LEN] = {0};
...
static xprs_ble_api_result_t ble_api_result_make(int status_code, char *body)
{
    xprs_ble_api_result_t result = {
...
{
    size_t max_messages = XPRS_BLE_CHAT_FETCH_MAX;
    ...
    size_t selected_indices[XPRS_BLE_CHAT_FETCH_MAX] = {0};
    size_t selected_count = 0;

    for (size_t i = 0; i < count && selected_count < XPRS_BLE_CHAT_FETCH_MAX; i++) {
    ...
    size_t pos = 0;
    char *response = malloc(XPRS_BLE_CHAT_JSON_MAX_LEN);
    if (!response) {
        free(messages);
        return ble_strdup_local("{\"messages\":[]}");
    }
    ...
    if (!ble_json_buffer_appendf(response, XPRS_BLE_CHAT_JSON_MAX_LEN, &pos, "{\"messages\":[")) {
        free(messages);
        free(response);
        return ble_strdup_local("{\"messages\":[]}");
    }
    ...
    for (size_t idx = start_index; idx < selected_count; idx++) {
        ...
        if (!first) {
            if (!ble_json_buffer_appendf(response, XPRS_BLE_CHAT_JSON_MAX_LEN, &pos, ",")) {
                free(messages);
                free(response);
                return ble_strdup_local("{\"messages\":[]}");
            }
        ...
        if (!ble_json_buffer_append_escaped(response, XPRS_BLE_CHAT_JSON_MAX_LEN, &pos, msg->text)) {
            free(messages);
            free(response);
            return ble_strdup_local("{\"messages\":[]}");
        }
        ...
        if (!ble_json_buffer_append_escaped(response, XPRS_BLE_CHAT_JSON_MAX_LEN, &pos, msg->callsign)) {
            free(messages);
            free(response);
            return ble_strdup_local("{\"messages\":[]}");
        }
        ...
        if (!ble_json_buffer_append_escaped(response, XPRS_BLE_CHAT_JSON_MAX_LEN, &pos, effective_room)) {
            free(messages);
            free(response);
            return ble_strdup_local("{\"messages\":[]}");
        }
        ...
        if (msg->msg_type == MESH_CHAT_MSG_FILE) {
            ...
            if (!ble_json_buffer_append_escaped(response, XPRS_BLE_CHAT_JSON_MAX_LEN, &pos,
                                                msg->file.filename)) {
                free(messages);
                free(response);
                return ble_strdup_local("{\"messages\":[]}");
...
static xprs_ble_api_result_t ble_handle_chat_message_post(cJSON *body,
                                                              const char *fallback_author,
                                                              const char *room)
{
    char author[XPRS_BLE_MAX_AUTHOR_LEN] = {0};
    char text[XPRS_BLE_MAX_MESSAGE_LEN + 1] = {0};
...
static xprs_ble_api_result_t ble_handle_chat_file_post(cJSON *body, const char *fallback_author)
{
    bool body_obj_borrowed = false;
    cJSON *body_obj = ble_parse_body_object(body, &body_obj_borrowed);
    if (!body_obj) {
...
static xprs_ble_api_result_t ble_dispatch_api_request(const char *method,
                                                         const char *path,
                                                         const char *query,
                                                         cJSON *body,
                                                         const char *peer_callsign)
{
    ...
    char room[XPRS_BLE_MAX_ROOM_LEN] = {0};
...
static int ble_notify_json(uint16_t conn_handle, const char *json)
{
    ...
    size_t chunk_size = XPRS_BLE_MAX_NOTIFY_CHUNK;
    if (mtu > 3) {
                                          ...
                                          const char *fallback_request_id)
{
    ...
    char normalized_path[XPRS_BLE_MAX_PATH_LEN] = {0};
    char query[XPRS_BLE_MAX_QUERY_LEN] = {0};
    ...
    xprs_ble_api_result_t result = ble_dispatch_api_request(
        method,
        normalized_path,
        query,
        body_item,
...
static void ble_forward_broadcast(uint16_t source_conn_handle, const char *json)
{
    ...
    for (size_t i = 0; i < XPRS_BLE_MAX_CONNECTIONS; i++) {
        xprs_ble_peer_t *peer = &s_peers[i];
                                 ...
                                 cJSON *message)
{
    ...
    if (content && content[0] != '\0') {
        char line[XPRS_BLE_MAX_MESSAGE_LEN + 1] = {0};
                            ...
                            cJSON *message)
{
    ...
    char effective_author[XPRS_BLE_MAX_AUTHOR_LEN] = {0};
...
static void ble_process_peer_buffer(xprs_ble_peer_t *peer)
{
                              ...
                              struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
        ...
        xprs_ble_peer_t *peer = ble_alloc_peer(conn_handle);
        if (!peer) {
        ...
        if (incoming_len > XPRS_BLE_RX_BUFFER_SIZE) {
            ESP_LOGW(TAG, "BLE write chunk too large (%u > %u), dropping buffer",
                     (unsigned)incoming_len, (unsigned)XPRS_BLE_RX_BUFFER_SIZE);
            peer->rx_len = 0;
            free(incoming);
            return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
        }

        if (peer->rx_len + incoming_len > XPRS_BLE_RX_BUFFER_SIZE) {
            ESP_LOGW(TAG, "BLE RX buffer overflow (%u + %u > %u), resetting buffer",
                     (unsigned)peer->rx_len, (unsigned)incoming_len,
                     (unsigned)XPRS_BLE_RX_BUFFER_SIZE);
            peer->rx_len = 0;
        }
    ...
    switch (event->type) {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status == 0) {
                xprs_ble_peer_t *peer = ble_alloc_peer(event->connect.conn_handle);
                if (!peer) {
        ...
        case BLE_GAP_EVENT_SUBSCRIBE: {
            xprs_ble_peer_t *peer = ble_find_peer(event->subscribe.conn_handle);
            if (!peer) {
...
esp_err_t xprs_ble_init(void)
{
    ...
    ESP_LOGI(TAG, "XPRS BLE initialized");
    return ESP_OK;
#endif
}
