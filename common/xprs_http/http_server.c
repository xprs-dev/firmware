...
static const char *CHAT_PAGE_HTML =
    ...
    "</style></head><body>"
    "<header><h1>XPRS</h1><button class=\"ham\" onclick=\"toggleMenu()\">&#9776;</button></header>"
...
static esp_err_t connect_post_handler(httpd_req_t *req)
{
    ...
    if (s_config_callback != NULL) {
        ...
        esp_err_t conn_err = xprs_wifi_connect_sta(ssid, password);
        httpd_resp_set_type(req, "application/json");
        if (conn_err != ESP_OK) {
...
{
    char response[128];
    snprintf(response, sizeof(response), "{\"status\":\"ok\",\"device\":\"xprs\"}");
...
static const char *OTA_PAGE_HTML =
    ...
    "<meta charset=\"utf-8\">"
    "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
    "<title>XPRS Firmware Update</title>"
    "<style>"
    "*{box-sizing:border-box;margin:0;padding:0}"
    "body{font-family:monospace;background:#1a1a2e;color:#e0e0e0;padding:12px;max-width:500px;margin:0 auto}"
    "h1{color:#00d4ff;font-size:1.3em;margin-bottom:12px}"
