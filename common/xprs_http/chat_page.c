...
static const char *LANDING_PAGE_HTML_SUFFIX =
    ...
    /* These key names live in the VISITOR'S browser, not in our tree: a
     * cookie and five localStorage slots, one of which holds their NOSTR
     * keys. Renaming them to xprs_* would not migrate anything -- it would
     * log every visitor out and orphan their stored keys and messages
     * behind names nothing reads any more. They keep the old name until
     * something reads both and writes the new one. */
