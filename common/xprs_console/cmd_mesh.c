...
{
    xprs_mesh_status_t status = xprs_mesh_get_status();

    if (console_get_output_mode() == CONSOLE_OUTPUT_JSON) {
        const char *status_str;
        switch (status) {
            case XPRS_MESH_STATUS_STOPPED:    status_str = "stopped"; break;
            case XPRS_MESH_STATUS_STARTED:    status_str = "started"; break;
            case XPRS_MESH_STATUS_CONNECTED:  status_str = "connected"; break;
            case XPRS_MESH_STATUS_DISCONNECTED: status_str = "disconnected"; break;
            case XPRS_MESH_STATUS_ROOT:       status_str = "root"; break;
            case XPRS_MESH_STATUS_ERROR:      status_str = "error"; break;
            default:                              status_str = "unknown"; break;
        }

        char ip_str[16] = {0};
        xprs_mesh_get_external_ap_ip(ip_str, sizeof(ip_str));

        printf("{\"status\":\"%s\",\"is_root\":%s,\"layer\":%d,\"nodes\":%zu,"
               "\"subnet_id\":%d,\"ip\":\"%s\",\"ap_running\":%s,\"ap_clients\":%d,"
               "\"bridge_enabled\":%s}\n",
               status_str,
               xprs_mesh_is_root() ? "true" : "false",
               xprs_mesh_get_layer(),
               xprs_mesh_get_node_count(),
               xprs_mesh_get_subnet_id(),
               ip_str,
               xprs_mesh_external_ap_is_running() ? "true" : "false",
               xprs_mesh_get_external_ap_client_count(),
               xprs_mesh_bridge_is_enabled() ? "true" : "false");
    } else {
        printf("\n=== Mesh Network Status ===\n");
        ...
        switch (status) {
            case XPRS_MESH_STATUS_STOPPED:    status_str = "Stopped"; break;
            case XPRS_MESH_STATUS_STARTED:    status_str = "Started (scanning)"; break;
            case XPRS_MESH_STATUS_CONNECTED:  status_str = "Connected"; break;
            case XPRS_MESH_STATUS_DISCONNECTED: status_str = "Disconnected"; break;
            case XPRS_MESH_STATUS_ROOT:       status_str = "Connected (ROOT)"; break;
            case XPRS_MESH_STATUS_ERROR:      status_str = "Error"; break;
            default:                              status_str = "Unknown"; break;
        }

        printf("Status:      %s\n", status_str);
        printf("Is Root:     %s\n", xprs_mesh_is_root() ? "YES" : "no");
        printf("Layer:       %d\n", xprs_mesh_get_layer());
        printf("Subnet ID:   %d (192.168.%d.0/24)\n",
               xprs_mesh_get_subnet_id(), 10 + xprs_mesh_get_subnet_id());
        printf("Node Count:  %zu\n", xprs_mesh_get_node_count());

        // Parent info
        uint8_t parent_mac[6];
        if (xprs_mesh_get_parent_mac(parent_mac) == ESP_OK) {
        ...
        printf("\n--- External AP ---\n");
        printf("Running:     %s\n", xprs_mesh_external_ap_is_running() ? "yes" : "no");
        if (xprs_mesh_external_ap_is_running()) {
            char ip_str[16] = {0};
            xprs_mesh_get_external_ap_ip(ip_str, sizeof(ip_str));
            printf("IP Address:  %s\n", ip_str);
            printf("Clients:     %d\n", xprs_mesh_get_external_ap_client_count());
        ...
        printf("\n--- IP Bridge ---\n");
        printf("Enabled:     %s\n", xprs_mesh_bridge_is_enabled() ? "yes" : "no");
        if (xprs_mesh_bridge_is_enabled()) {
            uint32_t pkts_tx, pkts_rx, bytes_tx, bytes_rx;
            xprs_mesh_bridge_get_stats(&pkts_tx, &pkts_rx, &bytes_tx, &bytes_rx);
...
static int cmd_mesh_start(int argc, char **argv)
{
    ...
    xprs_mesh_register_data_callback(mesh_test_rx_callback);

    ret = xprs_mesh_start(&cfg);
    if (ret != ESP_OK) {
...
{
    if (!xprs_mesh_is_connected() && xprs_mesh_get_status() == XPRS_MESH_STATUS_STOPPED) {
        printf("Mesh not running\n");
        return 0;
    }

    printf("Stopping mesh network...\n");
    esp_err_t ret = xprs_mesh_stop();
    if (ret != ESP_OK) {
        printf("Error: %s\n", esp_err_to_name(ret));
        return 1;
    }
...
{
    xprs_mesh_node_t nodes[20];
    size_t count = 0;

    esp_err_t ret = xprs_mesh_get_nodes(nodes, 20, &count);
    if (ret != ESP_OK) {
...
static int cmd_mesh_send(int argc, char **argv)
{
    ...
    esp_err_t ret = xprs_mesh_send_to_node(dest_mac, msg, total_len);
    free(msg);
...
static int cmd_mesh_broadcast(int argc, char **argv)
{
    ...
    xprs_mesh_node_t nodes[20];
    size_t count = 0;
    xprs_mesh_get_nodes(nodes, 20, &count);

    int success = 0, failed = 0;
    for (size_t i = 0; i < count; i++) {
        esp_err_t ret = xprs_mesh_send_to_node(nodes[i].mac, msg, total_len);
        if (ret == ESP_OK) {
            success++;
...
static int cmd_mesh_ping(int argc, char **argv)
{
    ...
    esp_err_t ret = xprs_mesh_send_to_node(dest_mac, &msg, sizeof(msg));
    if (ret != ESP_OK) {
...
static int cmd_mesh_bridge(int argc, char **argv)
{
    ...
    if (console_get_output_mode() == CONSOLE_OUTPUT_JSON) {
        ...
        printf("Status:      %s\n", xprs_mesh_bridge_is_enabled() ? "Enabled" : "Disabled");
        printf("Packets TX:  %lu\n", (unsigned long)pkts_tx);
        printf("Packets RX:  %lu\n", (unsigned long)pkts_rx);
        printf("Bytes TX:    %lu\n", (unsigned long)bytes_tx);
        printf("Bytes RX:    %lu\n", (unsigned long)bytes_rx);
...
static int cmd_mesh_ap(int argc, char **argv)
{
    ...
    if (mesh_ap_args.stop->count > 0) {
        ...
        esp_err_t ret = xprs_mesh_stop_external_ap();
        if (ret != ESP_OK) {
            printf("Error: %s\n", esp_err_to_name(ret));
            return 1;
        }
    ...
    // Build SSID: xprs-{callsign} or custom if provided
    char ap_ssid[32];
    ...
    esp_err_t ret = xprs_mesh_start_external_ap(ap_ssid, password, 4);
    if (ret != ESP_OK) {
        printf("Error: %s\n", esp_err_to_name(ret));
        return 1;
    }
    ...
    switch (msg->type) {
        case MESH_TEST_MSG_PING:
            ...
            // Send PONG back
            {
                ...
                xprs_mesh_send_to_node(src_mac, &pong, sizeof(pong));
            }
            break;
...
#else  // CONFIG_XPRS_MESH_ENABLED not defined
