...
static int cmd_status(int argc, char **argv)
{
    ...
    if (mode == CONSOLE_OUTPUT_JSON) {
        ...
        // WiFi status
        xprs_wifi_status_t wifi_status = xprs_wifi_get_status();
        char ip_str[16] = {0};
        if (wifi_status == XPRS_WIFI_STATUS_GOT_IP) {
            xprs_wifi_get_ip(ip_str);
            printf("\nWiFi: Connected (%s)\n", ip_str);
        } else if (xprs_wifi_is_ap_active()) {
        ...
        printf("BLE: %s\n", xprs_ble_is_running() ? "Advertising" : "Stopped");
...
{
    console_printf("version", "%s", XPRS_VERSION);
