...
static int cmd_ssh(int argc, char **argv)
{
    ...
    if (strcmp(action, "status") == 0) {
        // Show SSH status
        if (xprs_ssh_is_running()) {
            char fingerprint[64];
            printf("SSH Server: Running\n");
            printf("Port: %d\n", xprs_ssh_get_port());
            if (xprs_ssh_get_fingerprint(fingerprint) == ESP_OK) {
                printf("Fingerprint: %s\n", fingerprint);
            }
            printf("Password: %s\n", xprs_ssh_has_password() ? "Set" : "None (passwordless)");
        } else {
        ...
        if (ssh_args.password->count > 0) {
            ...
            if (strcmp(pw, "clear") == 0 || strcmp(pw, "none") == 0) {
                if (xprs_ssh_clear_password() == ESP_OK) {
                ...
                if (xprs_ssh_set_password(pw) == ESP_OK) {
            ...
            printf("\nCurrent: %s\n", xprs_ssh_has_password() ? "Password set" : "Passwordless");
        }
    }
    else {
