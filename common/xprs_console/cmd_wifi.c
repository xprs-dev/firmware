...
{
    xprs_wifi_status_t status = xprs_wifi_get_status();
    char ip_str[16] = {0};
    ...
    if (console_get_output_mode() == CONSOLE_OUTPUT_JSON) {
        ...
        switch (status) {
            case XPRS_WIFI_STATUS_GOT_IP:
                ...
                break;
            case XPRS_WIFI_STATUS_CONNECTING:
                status_str = "connecting";
                break;
            case XPRS_WIFI_STATUS_CONNECTED:
...
static int cmd_wifi_disconnect(int argc, char **argv)
{
    ...
    esp_err_t err = xprs_wifi_disconnect();
    if (err != ESP_OK) {
        printf("Error: %s\n", esp_err_to_name(err));
        return 1;
    }
...
static int cmd_wifi_saved(int argc, char **argv)
{
    ...
    esp_err_t err = xprs_wifi_load_credentials(ssid, password);
