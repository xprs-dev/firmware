...
/**
 * Parse manufacturer data looking for XPRS BlueAPRS marker.
...
static void handle_adv(const struct ble_gap_disc_desc *desc)
{
    ...
    /* Check XPRS marker */
    if (mfg[0] != COMPANY_ID_LO || mfg[1] != COMPANY_ID_HI || mfg[2] != XPRS_MARKER) return;
