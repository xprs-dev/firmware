...
#ifndef CONFIG_XPRS_MESH_BRIDGE_BUFFER_SIZE
#define CONFIG_XPRS_MESH_BRIDGE_BUFFER_SIZE 1500
#endif

#ifndef CONFIG_XPRS_MESH_BRIDGE_QUEUE_SIZE
#define CONFIG_XPRS_MESH_BRIDGE_QUEUE_SIZE 8
#endif
...
esp_err_t xprs_mesh_enable_bridge(void)
{
    ...
    xprs_mesh_register_data_callback(mesh_data_handler);
...
esp_err_t xprs_mesh_disable_bridge(void)
{
    ...
    xprs_mesh_register_data_callback(NULL);
...
esp_err_t mesh_bridge_forward_packet(uint32_t dest_ip, const uint8_t *data, size_t len)
{
    ...
    if (!data || len == 0 || len > CONFIG_XPRS_MESH_BRIDGE_BUFFER_SIZE) {
