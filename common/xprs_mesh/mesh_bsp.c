/**
 * @file mesh_bsp.c
 * @brief XPRS ESP-Mesh-Lite networking core implementation
 *
...
#ifndef CONFIG_XPRS_MESH_CHANNEL
#define CONFIG_XPRS_MESH_CHANNEL 1
#endif

#ifndef CONFIG_XPRS_MESH_MAX_LAYER
#define CONFIG_XPRS_MESH_MAX_LAYER 6
#endif

#ifndef CONFIG_XPRS_MESH_EXTERNAL_AP_MAX_CONN
#define CONFIG_XPRS_MESH_EXTERNAL_AP_MAX_CONN 4
#endif
...
static xprs_mesh_status_t s_status = XPRS_MESH_STATUS_STOPPED;
static xprs_mesh_event_cb_t s_event_callback = NULL;
static xprs_mesh_data_cb_t s_data_callback = NULL;

// Mesh configuration
static uint8_t s_mesh_id[6];
static uint8_t s_channel = CONFIG_XPRS_MESH_CHANNEL;
static uint8_t s_max_layer = CONFIG_XPRS_MESH_MAX_LAYER;
...
esp_err_t xprs_mesh_deinit(void)
{
    ...
    s_initialized = false;
    s_status = XPRS_MESH_STATUS_STOPPED;
...
esp_err_t xprs_mesh_start(const xprs_mesh_config_t *config)
{
    ...
    // router-less mode (expected in XPRS). Keep only errors for this tag.
    ...
    s_started = true;
    s_status = XPRS_MESH_STATUS_STARTED;
...
esp_err_t xprs_mesh_stop(void)
{
    ...
    if (s_event_callback) {
        s_event_callback(XPRS_MESH_EVENT_STOPPED, NULL);
    }
...
{
    return s_status == XPRS_MESH_STATUS_CONNECTED ||
           s_status == XPRS_MESH_STATUS_ROOT;
}
...
esp_err_t xprs_mesh_get_parent_mac(uint8_t *mac)
{
...
bool xprs_mesh_has_parent(void)
{
    return s_has_parent;
}

size_t xprs_mesh_get_peer_count(void)
{
    ...
    if (s_is_root && s_status == XPRS_MESH_STATUS_ROOT) {
...
esp_err_t xprs_mesh_start_external_ap(const char *ssid, const char *password,
...
bool xprs_mesh_external_ap_is_running(void)
{
...
esp_err_t xprs_mesh_get_external_ap_ip_addr(uint32_t *ip)
{
...
uint8_t xprs_mesh_get_external_ap_client_count(void)
{
...
esp_err_t xprs_mesh_get_nodes(xprs_mesh_node_t *nodes, size_t max_nodes,
                                  size_t *node_count)
{
...
size_t xprs_mesh_get_node_count(void)
{
    // Return at least 1 for self
    return 1;
}

esp_err_t xprs_mesh_find_node_by_subnet(uint8_t subnet_id, xprs_mesh_node_t *node)
{
...
esp_err_t xprs_mesh_send_to_node(const uint8_t *dest_mac, const void *data, size_t len)
{
    if (!dest_mac || !data || len == 0) {
        ESP_LOGE(TAG, "[TX] Invalid arguments");
        return ESP_ERR_INVALID_ARG;
    }
    if (!s_started || !xprs_mesh_is_connected()) {
...
void xprs_mesh_register_data_callback(xprs_mesh_data_cb_t callback)
{
                                    ...
                                    int32_t event_id, void *data)
{
    switch (event_id) {
        case ESP_MESH_LITE_EVENT_CORE_STARTED:
            ...
            s_status = XPRS_MESH_STATUS_STARTED;
            if (s_event_callback) {
                s_event_callback(XPRS_MESH_EVENT_STARTED, NULL);
            }
            break;
        ...
        case ESP_MESH_LITE_EVENT_NODE_JOIN: {
            ...
            s_status = s_is_root ? XPRS_MESH_STATUS_ROOT : XPRS_MESH_STATUS_CONNECTED;
            s_has_parent = !s_is_root;

            if (s_event_callback) {
                s_event_callback(XPRS_MESH_EVENT_CONNECTED, NULL);
            }
            break;
        }
        ...
        case ESP_MESH_LITE_EVENT_NODE_LEAVE:
            ...
            s_status = XPRS_MESH_STATUS_DISCONNECTED;
            s_has_parent = false;
            s_layer = 0;

            if (s_event_callback) {
                s_event_callback(XPRS_MESH_EVENT_DISCONNECTED, NULL);
            }
            break;
        ...
        case ESP_MESH_LITE_EVENT_NODE_CHANGE: {
            ...
            if (new_layer != s_layer || new_is_root != s_is_root) {
                ...
                if (s_event_callback) {
                    s_event_callback(XPRS_MESH_EVENT_ROOT_CHANGED, NULL);
                }
            }
            break;
        }
                             ...
                             int32_t event_id, void *event_data)
{
    switch (event_id) {
        case IP_EVENT_AP_STAIPASSIGNED: {
            ...
            if (s_event_callback) {
                xprs_mesh_external_sta_t sta = {0};
                memcpy(sta.mac, event->mac, 6);
                sta.ip = event->ip.addr;
                s_event_callback(XPRS_MESH_EVENT_EXTERNAL_STA_CONNECTED, &sta);
            }
            break;
        }
