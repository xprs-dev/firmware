...
size_t station_build_status_json(char *buffer, size_t size) {
    ...
    geo_json_add_bool(&builder, "mesh_connected", xprs_mesh_is_connected());
    geo_json_add_bool(&builder, "mesh_is_root", xprs_mesh_is_root());
    geo_json_add_int(&builder, "mesh_layer", xprs_mesh_get_layer());
    geo_json_add_int(&builder, "mesh_nodes", xprs_mesh_get_node_count());
#else
...
size_t station_build_device_json(char *buffer, size_t size) {
    ...
    int n = snprintf(buffer, size,
        "{\"id\":\"xprs-%02x%02x%02x%02x%02x%02x\","
