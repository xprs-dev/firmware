...
/* Longer APRS messages over BLE — same technique as xprs_ble_aprs
                                    ...
                                    uint8_t prio, uint32_t ttl_sec)
{
    ...
    for (int idx = 0; idx < total; idx++) {
        ...
        s->adv[a++] = XPRS_MARKER; s->adv[a++] = BCAST_PRIMARY;
        s->adv[a++] = s_tx_srctag;
        ...
        if (pcont > 0) {
            ...
            s->rsp[c++] = XPRS_MARKER; s->rsp[c++] = BCAST_CONT;
            s->rsp[c++] = s_tx_srctag;
...
static void advertise_with(const uint8_t *mfg, uint8_t mfg_len)
{
    ...
    cont[2] = XPRS_MARKER; cont[3] = APRS_CONT_SUBTYPE;
                         ...
                         struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    /* BLE parcel (xprs parcel protocol). Chat frames are a single parcel. */
    ble_parcel_hdr_t p;
...
static void brx_emit_nacks(void)
{
    ...
    for (int i = 0; i < BRX_SLOTS; i++) {
        ...
        frame[n++] = XPRS_MARKER; frame[n++] = BCAST_NACK;
...
static int ble_hello_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC: {
        ...
        if (mlen >= CONT_HDR_LEN && mfg[2] == XPRS_MARKER &&
        ...
        if (mlen >= 2 + BCH_NACK_HDR && mfg[2] == XPRS_MARKER &&
            mfg[3] == BCAST_NACK) {
            track_device(addr);
        ...
        if (mlen > 4 && mfg[2] == XPRS_MARKER && mfg[3] == XPRS_SUBTYPE) {
            track_device(addr);
            xprs_ingest(&mfg[4], mlen - 4, event->disc.rssi);
            break;
        }

        if (mfg[2] == XPRS_MARKER) {
            /* XPRS presence beacon: [company,marker,device_id,callsign…] */
            track_device(addr);
