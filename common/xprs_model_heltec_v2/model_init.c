...
static sx1276_handle_t s_lora = NULL;
...
float model_get_battery_voltage(void)
{
    ...
    // ESP32 ADC: 12-bit (0-4095), ~0-2.5V at 12dB attenuation
    // Battery divider: 100k + 220k -> scale = (100+220)/100 = 3.2, adjusted to 4.2
...
sx1276_handle_t model_get_lora(void)
{
    return s_lora;
}
...
esp_err_t model_init(void)
{
    ...
    // 5. Initialize SX1276 LoRa radio
    sx1276_spi_config_t lora_spi = {
        .mosi_pin = LORA_PIN_MOSI,
        .miso_pin = LORA_PIN_MISO,
        .sck_pin = LORA_PIN_SCK,
        .cs_pin = LORA_PIN_NSS,
        .rst_pin = LORA_PIN_RST,
        .dio0_pin = LORA_PIN_DIO0,
    };
    ret = sx1276_create(&lora_spi, &s_lora);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "SX1276 create failed: %s", esp_err_to_name(ret));
        ...
        ret = sx1276_init(s_lora, &lora_config);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "SX1276 init failed: %s", esp_err_to_name(ret));
            sx1276_delete(s_lora);
            s_lora = NULL;
        } else {
